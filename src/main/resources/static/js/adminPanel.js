...
$(function () {
    ...
    function updateUsers() {
        ...
        api.getUser().then(user_json => {
            ...
            head_text.text("Пользователь: " + user_json.email + " с ролями: " + roles_text);
        });
        ...
        api.getUsers().then(users_json => {
            for (let i = 0; i < users_json.length; i++) {
                ...
                roles_text = users_json[i].roles.map(r => r.name)
                    .map(r => r.replaceAll("ROLE_", "")).join(' ');
                ...
                let tr = $('<tr/>')
                    ...
                    .append($('<td/>').append($('<span/>').text(roles_text)))
                    .append($('<td/>')
                        .append('<button type="button" class="btn btn-info btn-sm ml-4 mr-2" ' +
                            'data-user = ' + JSON.stringify(user) + ' data-toggle="modal" ' +
                            'data-target="#updateModal"> Изменить </button>')

                        .append('<button type="button" class="btn btn-danger btn-sm"  ' +
                            'data-user = ' + JSON.stringify(user) + ' data-toggle="modal" ' +
                            'data-target="#deleteModal"> Удалить </button>')
                    );
                tbody.append(tr);
            }
        });

        //добавляем все возможные роли в селект на закладке нового пользователя,
        // в модальных окнах удаления и редактирования
    ...
    $('#button_new_user').click(function () {
        ...
        // заполняем нового пользователя данными
        // (у всех полей ввода на вкладке с id="newuser" атрибут class="form-control"
